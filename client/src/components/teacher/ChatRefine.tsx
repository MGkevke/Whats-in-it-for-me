...
export default function ChatRefine({ lessonId, onUpdate }: ChatRefineProps) {
  ...
  return (
    <div className="bg-white rounded-2xl border border-gray-100 shadow-sm flex flex-col h-[500px] lg:h-[600px]">
      {/* Header */}
      <div className="px-4 py-3 border-b border-gray-100 flex items-center gap-2">
        <MessageSquare className="w-4 h-4 text-purple-600" />
        <h3 className="text-sm font-semibold text-gray-900">Bijsturen met AI</h3>
      </div>
      ...
      <div className="flex-1 overflow-y-auto p-4 space-y-3">
        {messages.length === 0 && (
          <div className="text-center text-gray-400 text-sm py-8">
            ...
            <p className="text-xs mt-2 text-gray-300">bijv. "Maak de samenvatting korter"</p>
          </div>
        )}
      ...
      <div className="p-3 border-t border-gray-100">
        <div className="flex gap-2">
          <input
            ...
            className="flex-1 px-4 py-2.5 rounded-xl border border-gray-200 text-sm focus:border-purple-400 focus:ring-2 focus:ring-purple-100 focus:outline-none transition-all"
            disabled={sending}
          />
          <motion.button
